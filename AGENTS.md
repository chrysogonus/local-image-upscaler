...
| `models/manifest.json` | Metadata and checksums for optional runtimes and weights, plus the commit-pinned ComfyUI a fresh `make setup-comfyui` clones (binaries ignored). |
| `Dockerfile`, `docker-compose*.yml`, `docker/` | One digest-pinned CUDA-based image for every host, plus `docker-compose.gpu.yml` carrying only the device reservation Compose cannot make conditional; weights in a named volume, port on host loopback only. |
| `docs/` | The long-form manual: deployment, reference, development. `README.md` is the landing page and links here rather than absorbing it. |

- `make up`, then `make down`, `make logs`, `make shell` — the containerised app, and the
  only supported way to run it. `scripts/compose.sh` adds the GPU reservation when a real
  `--gpus all` container starts, never by reading what Docker reports about its runtimes,
  which is wrong on hosts that serve GPUs through CDI; `UPSCALER_GPU=0|1` forces it.
- `make setup-comfyui` — installs or adopts the ComfyUI the Illustration mode drives, and
  records it in `.upscaler/comfyui.conf` so no later command needs the path. Adopts one
  already on the machine in preference to cloning, and never moves an adopted checkout to
  the pinned revision. `make up` then starts it and `make down` stops it.
- `make setup-model-comfyui-illustration` — reinstalls just the illustration weight into
  the recorded ComfyUI. Normally covered by `setup-comfyui`.
- `make setup` — Python and frontend dependencies, for development and the gates only.
...
- `make clean-data COMFYUI=<path>` / `make clean-data-force` — erase every job workspace (host and Docker
  ...
  locates the install because it is a separate application this project does not manage;
  without it the ComfyUI half is skipped loudly. Browser local storage is out of reach and it says so.
